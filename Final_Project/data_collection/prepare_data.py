...
NEW: --plot shows the pulled window s and the recovered s_tilde overlaid, so you
can SEE before-vs-after. It also prints an s-fingerprint (mean / argmax / hash)
right after the window is pulled, so two runs are trivially comparable in the
console without eyeballing 256 floats. If the fingerprint is identical across
runs with different --record/--window, the data really isn't changing (most
likely the wfdb stream is silently failing -> synthetic fallback, which is
always seeded the same).
...
-----
    ...
    python3 prepare_data.py --plot                # pop up before/after plot
    python3 prepare_data.py --plot-save win.png   # save before/after plot to a PNG
"""
import argparse
import hashlib
import os
...
def fingerprint(s):
    """A short, run-to-run-comparable signature of a window.

    If two runs print the same fingerprint, the input window is literally
    identical -- so anything 'different' you expected downstream can't be coming
    from the data. This is the fastest way to catch the synthetic-fallback trap.
    """
    h = hashlib.md5(np.ascontiguousarray(s, dtype=np.float64).tobytes()).hexdigest()[:8]
    return f"mean={np.mean(s):+.4f}  R-peak@={int(np.argmax(np.abs(s)))}  hash={h}"
...
# ===========================================================================
# Plot: before (pulled window s) vs after (recovered s_tilde).
# ===========================================================================
def plot_before_after(s, s_tilde, p, src, show, save_path):
    try:
        import matplotlib
        if not show:                 # headless / save-only: no display needed
            matplotlib.use("Agg")
    ...
    # Top: just the pulled window, so you can confirm the INPUT changed.
    ax_top.plot(s, color="C0", lw=1.4)
    ax_top.set_title(f"BEFORE — pulled window\n{src}\n{fingerprint(s)}", fontsize=9)
    ax_top.set_ylabel("normalized amplitude")
    ax_top.grid(alpha=0.3)

    # Bottom: original vs recovered overlay.
    ax_bot.plot(s, color="C0", lw=1.6, label="original s")
    if s_tilde is not None:
        ax_bot.plot(s_tilde, color="C3", lw=1.2, ls="--",
                    label=f"recovered s\u0303  (PRD={p:.2f}%)")
    ax_bot.set_title("AFTER — original vs recovered", fontsize=10)
    ax_bot.set_xlabel("sample index (0..255)")
    ax_bot.set_ylabel("normalized amplitude")
...
def main():
    ...
    ap.add_argument("--plot", action="store_true",
                    help="pop up a before/after plot (needs matplotlib + a display)")
    ap.add_argument("--plot-save", default=None, metavar="PNG",
                    help="save the before/after plot to this PNG (works headless)")
    args = ap.parse_args()
    ...
    # Fingerprint right here: if this line is identical across two runs with
    # different --record/--window, the INPUT didn't change and the problem is
    # upstream of everything else (almost always the synthetic fallback firing).
    print(f"s fingerprint    : {fingerprint(s)}")
    ...
    # 6b. Plot before/after --------------------------------------------------
    if args.plot or args.plot_save:
        plot_before_after(s, s_tilde, p, src,
                           show=args.plot, save_path=args.plot_save)
